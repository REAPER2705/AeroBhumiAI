...
export default function NewAudit({ setActiveTab, initialStep = 'select' }: NewAuditProps) {
  ...
  if (step === 'report_and_flag') {
    ...
    return (
      <div className="p-8 max-w-2xl mx-auto bg-[#0f0f0f]">
        <h1 className="text-2xl font-bold text-white mb-2">Flag for Official Verification</h1>
        <p className="text-gray-400 mb-8 text-sm">Review the conflict evidence below and submit for government verification.</p>

        {error && <div className="mb-4 p-4 bg-[#ff3333] bg-opacity-20 text-[#ff3333] rounded-lg text-sm">{error}</div>}

        {/* Conflict Summary Box */}
        <div className="bg-[#2a2a2a] border border-[#3a3a3a] rounded-xl p-6 mb-8">
          <h2 className="text-lg font-bold text-white mb-4">Potential Boundary Conflict</h2>
          
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase mb-1">Affected Area</p>
              <p className="text-2xl font-bold text-[#ff3333]">{metrics.outside_area_m2?.toFixed(2)} m²</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-gray-400 uppercase mb-1">Outside Percentage</p>
              <p className="text-2xl font-bold text-[#ff3333]">{metrics.outside_percentage?.toFixed(2)}%</p>
            </div>
          </div>

          <div className="border-t border-[#3a3a3a] pt-4">
            <p className="text-xs font-semibold text-gray-400 uppercase mb-1">Why Flagged</p>
            <p className="text-sm text-gray-300">{auditResult?.problem || 'Portion of the proposed building extends outside the recorded parcel boundary.'}</p>
          </div>
        </div>

        {/* Spatial Verification Confidence */}
        <div className="bg-[#2a2a2a] border border-[#3a3a3a] rounded-xl p-6 mb-8">
          <h2 className="text-lg font-bold text-white mb-3">Spatial Verification Confidence</h2>
          
          <div className="flex items-center gap-4 mb-4">
            <div>
              <p className="text-sm font-semibold text-gray-400 mb-1">Confidence Score</p>
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-bold text-[#0066ff]">{confidence.score}</span>
          ...
          <div className="bg-[#1a1a1a] rounded-lg p-3 border border-[#3a3a3a]">
            <p className="text-xs font-semibold text-gray-300 mb-2">Evidence Factors:</p>
        ...
        <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-6 mb-8 shadow-sm">
          <h2 className="text-lg font-bold text-white mb-4">Conflict Evidence Snapshot</h2>

          {/* Show Map Snapshot if Available */}
          {(mapSnapshot || evidenceSnapshot) ? (
            <div className="mb-4">
              <div className="bg-[#2a2a2a] border border-[#2a2a2a] rounded-lg overflow-hidden max-h-64 flex items-center justify-center">
                <img
            ...
            <div className="bg-[#2a2a2a] border-2 border-dashed border-[#2a2a2a] rounded-lg p-8 text-center mb-4">
              <p className="text-sm text-gray-400">No snapshot uploaded. Please upload an evidence photo below.</p>
            </div>
          )}

          {/* Upload Evidence */}
          <div className="mb-4">
            <label className="block text-xs font-semibold text-gray-300 mb-2">Upload Evidence Snapshot (Optional)</label>
            <input
              type="file"
              accept="image/*"
              onChange={handleEvidenceUpload}
              className="w-full px-3 py-2 border border-[#2a2a2a] rounded-lg text-sm bg-[#2a2a2a] text-gray-300"
            />
          ...
          <div className="flex gap-2">
            {mapSnapshot || evidenceSnapshot ? (
              <button
                ...
                className="flex-1 px-3 py-2 bg-[#2a2a2a] text-gray-300 rounded-lg text-xs font-semibold hover:bg-[#333333]"
              >
                Replace Snapshot
              </button>
            ) : null}
